...
impl SlideLayer {
    ...
    pub fn display_name(&self) -> &'static str {
        match &self.content {
            ...
            LayerContent::Shape { shape_type, .. } => match shape_type {
...
impl Slide {
    ...
    pub fn new_text_in_group(text: String, group: String) -> Self {
        ...
        s.group_label = Some(group);
        s
    }

    pub fn effective_layers(&self) -> Cow<'_, [SlideLayer]> {
    ...
    pub fn migrate_to_layers(&mut self) -> bool {
        if !self.layers.is_empty() {
            return false;
        }
...
mod tests {
    ...
    fn effective_layers_with_no_layers_synthesises_one() {
        ...
        match &layers[0].content {
            LayerContent::Text { text, .. } => assert_eq!(text, "Hello"),
    ...
    fn effective_layers_with_layers_borrows_in_place() {
        ...
        slide
            .layers
            .push(SlideLayer::new_text("Layer A".to_string()));
        let layers = slide.effective_layers();
        assert_eq!(layers.len(), 1);
        match &layers[0].content {
            LayerContent::Text { text, .. } => assert_eq!(text, "Layer A"),
            _ => panic!("expected text layer"),
        }
    }
    ...
    fn effective_layers_with_layers_is_cow_borrowed() {
        let mut slide = make_text_slide();
        slide.layers.push(SlideLayer::new_text("x".to_string()));
        let layers = slide.effective_layers();
